...
json_values = json_values.map(function (item) { return [item[0], (item[1] / 100) / 100] })
...
function findDumpSheet() {
  ...
  const parentFolder = DriveApp.getFileById(activeSpreadsheet.getId()).getParents().next(); // selects folder where $activeSpreadsheet is
  const files = parentFolder.searchFiles(`title='${spreadsheetTitle}' and mimeType='${MimeType.GOOGLE_SHEETS}' and trashed=false`); // list files in $parentFolder with name $spreadSheetTitle
...
function get_data(desde, hasta) {
  ...
  if (!main_data) {
    ...
    url = url.concat(first_date, "&end_date=", last_date, "&ids=", key, "&format=json&metadata=none");
